        ...
        <div className="mt-8 p-6 bg-base rounded-lg shadow">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex items-center">
                        {[1, 2, 3, 4, 5].map(star => (
                            <button key={star} type="button" onClick={() => setRating(star)} className="p-1">
                                <Star
                                    className={`w-4 h-4 sm:w-6 sm:h-6 ${rating >= star ? 'text-yellow-400' : 'text-slate-300'}`}
                                    fill="currentColor"
                                />
                            </button>
                        ))}
                ...
                <Button type="submit" disabled={isSubmitting} className="flex items-center gap-2">
                    <Send className="w-4 h-4 sm:w-4 sm:h-4" /> {isSubmitting ? 'Submitting...' : 'Submit Review'}
                </Button>
            </form>
        </div>
    )
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div className="flex justify-between items-center mb-6">
                <button
                    onClick={() => navigate(-1)}
                    className="inline-flex items-center gap-2 text-text-secondary hover:text-primary transition-colors font-semibold text-sm"
                    aria-label="Go back to previous page"
                >
                    <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5" /> Back
                </button>
                <Link to="/" className="inline-flex items-center gap-2 text-text-secondary hover:text-primary transition-colors font-semibold text-sm">
                    <Home className="w-4 h-4 sm:w-5 sm:h-5" /> Back to Homepage
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div>
                    <div className="relative bg-base rounded-xl shadow-lg overflow-hidden mb-4">
                        <img
                            ...
                            alt={product.name}
                            className="w-full h-64 sm:h-96 object-cover animate-fade-in"
                        />
                        {product.images && product.images.length > 1 && (
                            <>
                                <button onClick={prevImage} aria-label="Previous image" className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/50 hover:bg-white/80 text-text-primary p-1.5 sm:p-2 rounded-full shadow-md transition-all">
                                    <ChevronLeft className="w-4 h-4 sm:w-6 sm:h-6" />
                                </button>
                                <button onClick={nextImage} aria-label="Next image" className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/50 hover:bg-white/80 text-text-primary p-1.5 sm:p-2 rounded-full shadow-md transition-all">
                    ...
                    <div className="flex gap-4">
                        {product.images?.map((img, index) => (
                            <button
                                ...
                                className={`w-16 h-16 sm:w-24 sm:h-24 rounded-lg overflow-hidden border-2 transition-all ${currentImageIndex === index ? 'border-primary' : 'border-transparent'}`}
                            >
                ...
                <div>
                    ...
                    <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold my-2">{product.name}</h1>
                    <div className="flex items-center gap-4 my-4">
                        <div className="flex text-yellow-400">
                            {[...Array(5)].map((_, i) => (
                                <Star key={i} className={`w-3 h-3 sm:w-5 sm:h-5 ${i < Math.round(product.rating) ? 'text-current' : 'text-transparent'}`} fill={i < Math.round(product.rating) ? 'currentColor' : 'none'} />
                            ))}
                        </div>
                        <span className="text-text-secondary">{(product.rating || 0).toFixed(1)} stars ({reviews.length} reviews)</span>
                    </div>
                    <p className="text-text-secondary text-base sm:text-lg leading-relaxed mb-6">{product.description}</p>

                    <div className="flex items-baseline gap-4 mb-6">
                        <p className={`text-2xl sm:text-4xl md:text-5xl font-bold ${product.sale_price ? 'text-secondary' : 'text-primary'}`}>
                            {formatNaira(displayPrice)}
                        </p>
                        {product.sale_price && (
                            <p className="text-base sm:text-2xl text-text-secondary line-through">
                                {formatNaira(product.price)}
                            </p>
                        )}
                    </div>

                    <div className={`mb-6 text-sm sm:text-lg font-semibold ${hasStock ? 'text-accent' : 'text-red-500'}`}>
                    ...
                    {hasStock && (
                        <div className="flex items-stretch gap-4 mb-6">
                            <div className="flex items-center border border-slate-300 rounded-lg">
                                <button onClick={decrementQuantity} className="p-2 sm:p-3 text-text-secondary hover:bg-neutral rounded-l-lg">
                                    <Minus className="w-3 h-3 sm:w-4 sm:h-4" />
                                </button>
                                <span className="px-3 sm:px-4 font-bold text-base sm:text-lg">{quantity}</span>
                                <button onClick={incrementQuantity} className="p-2 sm:p-3 text-text-secondary hover:bg-neutral rounded-r-lg">
                                    <Plus className="w-3 h-3 sm:w-4 sm:h-4" />
                                </button>
                            </div>
                            <Button size="lg" onClick={handleAddToCart} className="flex-grow flex items-center justify-center gap-2 px-3 sm:px-6 py-2 sm:py-3 text-sm sm:text-base">
                                {addedToCart ? <><CheckCircle className="w-4 h-4 sm:w-5 sm:h-5" /> Added!</> : 'Add to Cart'}
                            </Button>
                            <Button size="lg" variant="outline" onClick={handleToggleWishlist} className="px-3 sm:px-4 py-2 sm:py-3">
                                <Heart className={`w-4 h-4 sm:w-5 sm:h-5 ${wishlistDocId ? 'text-current' : 'text-transparent'}`} fill={wishlistDocId ? 'currentColor' : 'none'} />
                            </Button>
                        </div>
                    )}
                </div>
            ...
            <div className="mt-16 pt-10 border-t">
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-6">Customer Reviews</h2>
                {reviews.length > 0 ? (
                        ...
                        {reviews.map(review => (
                            <div key={review.id} className="p-4 border rounded-lg bg-base">
                                <div className="flex items-center gap-4 mb-2">
                                    ...
                                    <div className="flex text-yellow-400">
                                        {[...Array(5)].map((_, i) => <Star key={i} className={`w-3 h-3 sm:w-4 sm:h-4 ${i < review.rating ? 'text-current' : 'text-transparent'}`} fill={i < review.rating ? 'currentColor' : 'none'} />)}
                                    </div>
                                </div>
